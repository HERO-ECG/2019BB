...
void GenerallySetShootControl(RC_Ctl_t rc_data, RoboControlMode_t robo_ctrlmode, AmmunitionControl_t *AmmunitionControl)
{
	...
	{
		if(rc_data.mouse.press_1 == 1) {	AmmunitionControl->shoot17_order = Order_start1; AmmunitionControl->shoot42_order = Order_start1; }
		else { AmmunitionControl->shoot17_order = Order_stop; AmmunitionControl->shoot42_order = Order_stop; }
	}
	else //if(RoboData.robo_ctrlmode.ctrl_source == FROM_REMOTE)
	{
		if(robo_ctrlmode.left_mode == LEFT_down) { AmmunitionControl->shoot17_order = Order_start1; AmmunitionControl->shoot42_order = Order_start1; }
		else {	AmmunitionControl->shoot17_order = Order_stop; AmmunitionControl->shoot42_order = Order_stop; }
	}
