...
void PidSet_errband(Pid_t *pid, float max_err, float mid_err, float min_err, float deadband, float f_divider)
{
  pid->max_err = ABS(max_err);
	pid->mid_err = ABS(mid_err);
	pid->min_err = ABS(min_err);
...
float PidCalc(Pid_t* pid, float get, float set)
{
	...
	{ 			
		...
		/*前馈(速度环主用)*//*f分离*/
		if(ABS(set) > ABS(pid->f_divider)){
			pid->output.fout = pid->f * set;	
			ABSLimit(&(pid->output.fout),pid->f_out_limit);
		}
		else{ pid->output.fout =ABS(set)/(ABS(pid->f_divider)+1)*pid->f * set; }
		...
		{				
			if(abs_err > i_max_err)  {  pid->output.iout += pid->i * pid->err.now/2.0f; }
